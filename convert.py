...
class DICOM_Dataset(object):
    ...
    def __init__(self, input_dir, output_dir='', post_process=False):
        ...
        for p in sorted(patients):
            ...
            if self.cvi42_lb:
                ...
                self.avail_cont = [f.rstrip('.pickle') for f in os.listdir(contour_dir)]
    ...
    def process_series(self, dicom_dir, s):
        SKIP = False
        ...
        if prefix in self.processed:
            print('Series already considered! {}. Skipping!'.format(prefix))
            raise ValueError('Series already considered!')
        self.processed.append(prefix)
        ...
        if T*Z < len(files):
            ...
            # TODO
            raise ValueError('DICOM files seem to come from different cine images or have different time steps.')
        ...
        if not ds.__contains__('SeriesDescription') and not ds.__contains__('SequenceName'):
            ...
            serDesc = ds.SeriesDescription if ds.__contains__('SeriesDescription') else ds.SequenceName
        if serDesc == 'MESURES':
            raise ValueError('File not considered!')
        serDesc = regex.sub( r' |-|\\|\/', '_', serDesc)
        serDesc = regex.sub(r'\(|\)', '', serDesc)
        ...
        for idx, fi in enumerate(file_list):
            ...
            try:
                ...
                if volume[..., idx%Z, idx//Z].shape == im.shape:
                    ...
                    SKIP = True # Skip series
                    return None, None
            if ds.SOPInstanceUID in self.avail_cont:
        ...
        if hasattr(ds, 'SpacingBetweenSlices'):
            ...
            if ds.__contains__('SliceThickness'):
                ...
                raise ValueError('AttributeError: No SliceThickness attribute found. Skipping image...')
        ...
        try:
            ...
            raise ValueError('Error! Failed to create nifti file. Skipping image...')
        nii.header['pixdim'][4] = dt
        nii.header['sform_code'] = 1
        nib.save(nii, os.path.join(self.output_dir, s, '{}.nii.gz'.format(serDesc)))
        self.aux_file['file_path'] = os.path.join(self.output_dir, s, '{}.nii.gz'.format(serDesc))
        ...
        if self.cvi42_lb:
            ...
            self.aux_file['mask_path'] = os.path.join(self.output_dir, s, '{}_label.nii.gz'.format(serDesc))
            self.aux_file['upsample_mask_path'] = os.path.join(self.output_dir, s, '{}_label_upsample.nii.gz'.format(serDesc))
    ...
    def transformDicoms(self, dicom_dir, patient):
        ...
        for s in sorted(os.listdir(dicom_dir)):
            if not os.path.isdir(os.path.join(dicom_dir, s)):
                continue
            print('Series', s)
            if s == 'contours':
                continue
            ...
            try:
                ds, serDesc = self.process_series(dicom_dir, s)
            except Exception as e:
                print('Exception found!', e, 'Skip series', s)
                continue
        ...
        self.aux_file.update(info)
        self.patient_dict[patient].append(self.aux_file)
